...
class Logging
{
    ...
    {
        $result=DBManager::Execute(true, "SELECT `ip` FROM `" . DB_PREFIX . DATABASE_ADMINISTRATION_LOG . "` WHERE (`type` LIKE '%Upload::%' OR `type` LIKE '%::CreateFile%') AND `ip`='" . DBManager::RealEscape(Communication::GetIP(false)) . "' AND `time`>" . intval(time() - 86400));
        if(($rc=DBManager::GetRowCount($result)) >= MAX_FILE_UPLOADS_PER_DAY)
        {
